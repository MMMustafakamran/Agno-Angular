 ...
 *
 * Thread endpoints come from CopilotKit Intelligence. Without a
...
export const runThreadsAction: PageActionHandler = async (
  ...
  await typeInNotepad(
    ...
    [
      ...
      '',
      '- injectThreads list stays on "Loading conversations..."',
      '- the drawer renders its locked state, listing nothing',
    ],
    1550,
    280,
  );
  ...
  await typeInNotepad(
    ...
    [
      '',
      '- the chat answers fine; only persistence is missing',
      '- no intelligence key, so thread create/list/mutate',
      '  never reach a store. not a broken agent.',
      '',
      'pkgs: @copilotkit/angular 0.3.1, runtime 1.68.1,',
      '@angular/cdk 22',
    ],
    1550,
    380,
