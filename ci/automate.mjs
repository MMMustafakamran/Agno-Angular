...
console.log('═══════════════════════════════════════════════════════════════');
console.log('  🚀 CopilotKit Agno + Angular Automation Pipeline');
console.log('═══════════════════════════════════════════════════════════════');
  ...
  try {
    ...
    if (!skipInstall) {
      runSync(
        ...
        BACKEND_DIR,
        'Syncing Agent Dependencies (uv sync)',
      );
    ...
    if (busy.backend) {
      console.log('\n▶ [Step] Agno agent already running; reusing it.');
    } else {
      console.log('\n▶ [Step] Starting Agno Agent...');
    ...
    // 5. Health — the agent first: the runtime's /info is only meaningful once
    // there is an agent behind it.
    reportData.health.backend = (
      await waitForHealth(BACKEND_HEALTH_URL, 'Agno Agent', backendLog, 60000)
    ).elapsedSec;
