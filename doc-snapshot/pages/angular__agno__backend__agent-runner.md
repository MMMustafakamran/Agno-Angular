...
[HTTP routes](/angular/agno/backend/runtime-endpoints):
...
See [Thread authorization](/angular/agno/auth#thread-authorization) for the ownership-table
...
existing runtime, see [Connect your runtime to Intelligence](/angular/agno/intelligence/connect-your-runtime).
</Callout>
...
- **Whole-thread eviction** drops the entire least-recently-used thread — every
  ...
  this way no longer appears in [`GET /threads`](/angular/agno/backend/runtime-endpoints), and
...
[AWS AgentCore integration](/angular/agno/deploy/agentcore). It extends
...
sends a message. See the [`/connect` 404 troubleshooting entry](/angular/agno/troubleshooting/common-issues#connect-route-returns-404-on-a-fresh-thread).
</Callout>

## Related

- [Runtime HTTP endpoints](/angular/agno/backend/runtime-endpoints): the routes each runner method backs.
- [Copilot Runtime](/angular/agno/backend/copilot-runtime): configuring the runtime and `runner`.
- [AWS AgentCore](/angular/agno/deploy/agentcore): a custom runner subclass for an external memory layer.
- [Common issues](/angular/agno/troubleshooting/common-issues#runtime-memory-keeps-growing-or-the-process-runs-out-of-heap): diagnosing runtime memory growth and eviction warnings.
- [Self-host CopilotKit Intelligence](/angular/agno/intelligence/self-hosting): the durable, multi-instance `IntelligenceAgentRunner` backend.
