...
The frontend setup is the same as [BuiltInAgent](/angular/agno/quickstart): configure the
...
[AgentRunner & persistence](/angular/agno/backend/agent-runner).
...
With `type: "custom"`, you yield AG-UI events directly. See the [AG-UI event reference](/angular/agno/backend/ag-ui) for all available event types.
</Tab>
</Tabs>
...
Using `BuiltInAgent` without a custom factory? The same options go through its `providerOptions` — see [Advanced Configuration](/angular/agno/advanced-configuration#provider-specific-options).
</Tab>
<Tab value="TanStack AI">
<Callout type="warn">
