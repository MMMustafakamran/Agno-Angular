...
![The cloud-hosted Intelligence ready page.](/angular/agno/images/cloud-hosted/cloud-hosted-ready.png)
...
<Steps>
  <Step>
    ...
    A new organization chooses the Developer plan or a paid plan. Developer is the no-cost plan. Read [Plans](/angular/agno/intelligence/plans) when you want to change that choice.
  </Step>
  ...
  <Step>
    ...
    Sign in and select the project in the [quickstart](/angular/agno/intelligence/quickstart#select-an-intelligence-project). That page writes `CPK_INTELLIGENCE_API_KEY` to `.env`.
...
The [CopilotKit CLI](/angular/agno/cli) uses the same sign-in. The first `login` opens a browser sign-in page and stores a local session. Later commands reuse that session. `project select` creates the project API key and writes it to `.env`.

![The cloud-hosted project list.](/angular/agno/images/cloud-hosted/cloud-hosted-projects.png)
...
- **Project resources** contains [Threads](/angular/agno/guides/threads-memory-attachments-headless), [Channels](/angular/agno/intelligence/channels), [User Memories](/angular/agno/intelligence/memories), and [API Keys](#create-an-api-key).
...
![A cloud-hosted project and its threads.](/angular/agno/images/cloud-hosted/cloud-hosted-thread-list.png)

Open one thread to read its event timeline.

![A cloud-hosted thread with its event timeline.](/angular/agno/images/cloud-hosted/cloud-hosted-thread-detail.png)

The thread page shows the agent, the app user, the status, the last update time, and the event timeline. Open an event when you need the raw payload. **Rename**, **Archive**, and **Delete** match the thread API in your app: archive hides the thread from the active list and keeps the history, delete removes it. Read [Rich Threads](/angular/agno/guides/threads-memory-attachments-headless) for how a thread works in your app.

## Create an API key

A project API key connects your runtime to this project. The [quickstart](/angular/agno/intelligence/quickstart#select-an-intelligence-project) writes it to `.env` as `CPK_INTELLIGENCE_API_KEY`. `init` or its `create` alias writes the same key when it creates an app.
...
![The cloud-hosted API keys page.](/angular/agno/images/cloud-hosted/cloud-hosted-api-keys.png)
...
Connect the key in the [quickstart](/angular/agno/intelligence/quickstart).

## Next

- [Plans](/angular/agno/intelligence/plans) shows limits and how to change the plan.
- [Self-host Intelligence](/angular/agno/intelligence/self-hosting) when the platform must run in your cluster.
