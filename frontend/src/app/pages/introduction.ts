    ...
    <div class="space-y-6">
      ...
      <ui-panel heading="What this is">
        <p class="text-sm text-slate-700">
          A navigable test harness for the Angular + Agno section of the
...
OpenAI  (gpt-5.4-mini)</code></pre>
      ...
      <ui-callout title="Both backends must be running">
        The chat will not stream if either process is down. Start the Agno agent
        with <code>uv run main.py</code> from <code>backend/</code>, and the
      ...
      <ui-panel heading="The runtime binding">
        <p class="mb-3 text-sm text-slate-700">
          This is the one file that ties CopilotKit to Agno. It is read off disk
